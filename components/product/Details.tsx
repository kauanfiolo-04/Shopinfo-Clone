...
function ProductInfo({ page, pix, flags }: Props) {
    ...
    useEffect(()=>{
      (async()=>{
        setRenderizado(true)
        const { products_rates }=await invoke['deco-sites/shp'].loaders.getTrustvox({productId:isVariantOf!.productGroupID, storeId:'trustvoxStoreId'}) 
    ...
    const handleCadastro=async (event:Event)=>{
      ...
      if(inputNome.current && inputEmail.current && inputSku.current){
        ...
        if(skuId!=='' && name!=='' && email!==''){
          ...
          const data=await fetch('apiInternaSHP/notifyMe.php', {
            "body": formData,
            "method": "POST",
            "mode": "cors",
